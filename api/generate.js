...
function buildSubjectsPrompt(p) {
  ...
  const archiveSection = archive.length > 0
    ? `\n[과거 회차 아카이브 — 아래 소재·구조·감정 패턴 모두 이번 주제에서 절대 반복 금지]
${archive.map((a, i) => {
  const parts = [];
  if (a.date)             parts.push(`날짜:${a.date}`);
  if (a.kakaoTitle)       parts.push(`제목:「${a.kakaoTitle}」`);
  if (a.topic)            parts.push(`핵심주제:${a.topic}`);
  if (a.storySummary)     parts.push(`사연소재:${a.storySummary.slice(0, 60)}`);
  if (a.tipSummary)       parts.push(`꿀팁소재:${a.tipSummary.slice(0, 60)}`);
  if (a.quizSummary)      parts.push(`퀴즈소재:${a.quizSummary.slice(0, 40)}`);
  if (a.avoidExpressions) parts.push(`피할표현:${a.avoidExpressions.slice(0, 60)}`);
  return `[${i + 1}] ${parts.join(' | ')}`;
}).join('\n')}

[아카이브 기반 중복 판단 기준]
아래 중 하나라도 해당하면 duplicate_risk = "높음"으로 판단하고 다른 주제로 대체:
① 카카오 제목과 핵심 단어 3개 이상 겹침
② 사연 소재 업종·상황이 동일하거나 업종만 바꾼 수준
③ 꿀팁 소재 키워드가 2개 이상 동일
④ 감정 패턴이 동일 ("몰라서 손해", "억울한 세금", "끝난 줄 알았는데" 등)
⑤ 세무 핵심 질문이 같은 결론(비용처리 여부·신고 실수·환급 여부)으로 귀결

[강하게 피해야 할 반복 소재·표현 — 아카이브에 없어도 최근 반복됐다면 제외]
× 세금 폭탄 / 비용처리 / 경비 처리 / 카드값·세금계산서 정리
× 개인 비용과 사업 비용 구분 / 억울한 세금 / 몰라서 더 냄
× 뭘 먼저 봐야 하나요? / 비용 누락 / 종소세·부가세 기본 신고 실수
→ 이 단어가 있다고 무조건 금지는 아니지만, 이미 아카이브에 반복된 경우엔 같은 관점으로 추천하지 마세요.
→ 제목만 다르고 결국 "비용처리 이야기", "세금폭탄 이야기", "매번 몰라서 손해 구조"는 모두 "높음".

[duplicate_risk 규칙 — 필수 준수]
최종 출력되는 5개 subjects는 모두 duplicate_risk가 "낮음" 또는 "보통"이어야 합니다.
"높음"이 되는 아이디어는 버리고 새로운 관점의 주제로 교체하세요.\n`
  ...
  "subjects": [
    {
      ...
      "type": "나해당형",
      "duplicate_risk": "낮음",
      "similar_archive": "없음 — 또는 유사 과거 회차 번호/제목 (예: #8 택배비/비용처리)",
      "why_not_duplicate": "기존 회차와 어떤 점이 다른지 한 줄 설명",
...
[배열 개수]
subjects: 정확히 5개 (모두 duplicate_risk "낮음" 또는 "보통")
