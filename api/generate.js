...
function buildSubjectsPrompt(p) {
  ...
  const interestList = Array.isArray(p.currentInterest) ? p.currentInterest.join(', ') : (p.currentInterest || '없음');
...
[현재 시기 관심사 범주 — 주제 선정 참고]
${interestList}
위 범주 중 직전 소재와 겹치지 않는 항목을 중심으로 주제 후보를 구성하세요.
...
- 현재 시기 관심사 범주(${interestList})를 적극 반영하되, 직전 소재와 겹치는 범주 제외
